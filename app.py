...
def pct(numerator, denominator):
    return round((float(numerator or 0) / float(denominator or 1)) * 100, 2)

def fetch_scalar(sql, params=(), default=0):
    try:
        cursor.execute(sql, params)
        row = cursor.fetchone()
        return row[0] if row and row[0] is not None else default
    except Exception as e:
        print("Analytics scalar warning:", e)
        return default
...
def fetch_rows(sql, params=()):
    try:
        ...
        print("Analytics rows warning:", e)
        return []
...
def build_vendor_intelligence():
    vendors = fetch_rows("""
        SELECT
            ...
            COALESCE(SUM(CASE WHEN v.status='Approved' THEN 1 ELSE 0 END), 0),
            COALESCE(SUM(CASE WHEN v.status='Rejected' THEN 1 ELSE 0 END), 0),
            COALESCE(AVG(v.accuracy), 0),
            COALESCE(MAX(v.upload_date), NULL)
        FROM users u
        LEFT JOIN vendor_uploads v ON v.uploaded_by = u.id OR v.vendor_name = u.username
        WHERE u.role IN ('vendor', 'admin')
        GROUP BY u.id, u.username, u.role
    """)
    dpr_rows = fetch_rows("""
        SELECT vendor_name, COUNT(*), MAX(upload_date)
        FROM progress_uploads
        GROUP BY vendor_name
    """)
    dpr_map = {row[0]: {"count": row[1] or 0, "last": row[2]} for row in dpr_rows}
    ranked = []
    for row in vendors:
        user_id, username, role, drawings, approved, rejected, avg_accuracy, last_upload = row
        dpr_count = dpr_map.get(username, {}).get("count", 0)
        approval_rate = pct(approved, drawings)
        rejection_rate = pct(rejected, drawings)
        productivity = clamp((drawings * 12) + (dpr_count * 8))
        quality = clamp(float(avg_accuracy or 0))
        compliance = clamp((approval_rate * 0.75) + (quality * 0.25))
        safety = clamp(100 - (rejection_rate * 1.8))
        dpr_performance = clamp(dpr_count * 12)
        overall = round((productivity * .18) + (quality * .24) + (compliance * .24) + (safety * .16) + (dpr_performance * .18), 2)
        ranked.append({
            "id": user_id,
            "vendor": username,
            "role": role,
            ...
            "dpr_performance": round(dpr_performance, 2),
    ...
    for index, vendor in enumerate(ranked, 1):
        vendor["rank"] = index
    return ranked
...
def build_resource_summary():
    ...
    for row in resources:
        allocated = float(row[2] or 0)
        utilized = float(row[3] or 0)
        utilization = pct(utilized, allocated)
        items.append({
            "type": row[0],
            "name": row[1],
            "allocated": allocated,
            ...
            "shortage": float(row[5] or 0),
            "performance": round(float(row[6] or 0), 2),
            "utilization": utilization
        })
    shortage_count = sum(1 for item in items if item["shortage"] > 0)
    avg_utilization = round(sum(item["utilization"] for item in items) / max(len(items), 1), 2)
    avg_performance = round(sum(item["performance"] for item in items) / max(len(items), 1), 2)
...
def build_budget_summary():
    ...
    """)
    departments = []
    total_allocated = total_utilized = total_forecast = 0.0
    for row in rows:
        allocated = float(row[1] or 0)
        utilized = float(row[2] or 0)
        forecast = float(row[3] or 0)
        variance = forecast - allocated
        departments.append({
            "department": row[0],
            "allocated": allocated,
            "utilized": utilized,
            ...
            "variance": variance,
            "utilization": pct(utilized, allocated),
            "health": "Critical" if variance > allocated * .08 else ("Watch" if variance > 0 else "Healthy")
        })
    ...
    return {
        ...
        "remaining": total_allocated - total_utilized,
        "variance": total_forecast - total_allocated,
        "utilization": pct(total_utilized, total_allocated)
    }

def build_completion_forecast():
    rows = fetch_rows("""
        SELECT logged_date, AVG(completion_percentage), SUM(today_progress)
    ...
    if not rows:
        return {
            "estimated_completion": "Insufficient DPR history",
    ...
    first_date = rows[0][0]
    last_date = rows[-1][0]
    completion = round(float(rows[-1][1] or 0), 2)
    elapsed = max((last_date - first_date).days, 1) if hasattr(last_date, "__sub__") else max(len(rows), 1)
    daily_gain = max(completion / elapsed, 0.1)
    remaining_days = int(clamp(round((100 - completion) / daily_gain), 0, 730))
    estimated = last_date + timedelta(days=remaining_days) if hasattr(last_date, "__add__") else datetime.now().date() + timedelta(days=remaining_days)
    delay_probability = clamp(100 - (daily_gain * 18) + max(0, 70 - completion) * .35)
    activity_rows = fetch_rows("""
        SELECT activity_name, MAX(completion_percentage), MAX(logged_date)
        FROM running_progress_metrics
        GROUP BY activity_name
        ORDER BY MAX(completion_percentage) ASC
        LIMIT 10
    """)
    activity_forecasts = []
    for activity, act_completion, act_date in activity_rows:
        act_completion = float(act_completion or 0)
        act_days = int(clamp(round((100 - act_completion) / daily_gain), 0, 730))
        finish = act_date + timedelta(days=act_days) if hasattr(act_date, "__add__") else estimated
        activity_forecasts.append({
            "activity": activity,
            "completion": round(act_completion, 2),
            "forecast": finish.strftime("%d-%b-%Y") if hasattr(finish, "strftime") else str(finish)
        })
    return {
        "estimated_completion": estimated.strftime("%d-%b-%Y") if hasattr(estimated, "strftime") else str(estimated),
        "remaining_duration": f"{remaining_days} days",
        "delay_probability": round(delay_probability, 2),
        "overall_completion": completion,
        "activity_forecasts": activity_forecasts
    }

def build_document_intelligence():
    rows = fetch_rows("""
        SELECT pdf_name, vendor_name, status, accuracy, upload_date
        FROM vendor_uploads
        ORDER BY upload_date DESC
    """)
    grouped = {}
    for name, vendor, status, accuracy, upload_date in rows:
        base = re.sub(r"[_ -]?\d+$", "", os.path.splitext(name or "")[0]).lower()
        grouped.setdefault(base or name or "document", []).append({
            "name": name,
            "vendor": vendor,
            "status": status,
    ...
    for key, docs in grouped.items():
        docs.sort(key=lambda item: item["date"] or datetime.min, reverse=True)
        latest = docs[0]
        histories.append({
            "document": latest["name"],
            "vendor": latest["vendor"],
            "versions": len(docs),
            "latest_status": latest["status"],
            "latest_accuracy": latest["accuracy"],
            "relationship": "Revision series" if len(docs) > 1 else "Single submission",
            "compliance": "High" if latest["accuracy"] >= 75 else ("Medium" if latest["accuracy"] >= 55 else "Low")
        })
    return histories

def build_learning_analytics():
    rows = fetch_rows("""
        SELECT document_name, document_type, uploaded_by, upload_date, CHAR_LENGTH(extracted_text)
        FROM ai_learning
        ORDER BY upload_date DESC
    """)
    by_type = {}
    total_chars = 0
    for row in rows:
        doc_type = row[1] or "Unclassified"
        by_type[doc_type] = by_type.get(doc_type, 0) + 1
        total_chars += int(row[4] or 0)
    quality = clamp((len(rows) * 8) + (total_chars / 2500))
    return {
        "documents": rows,
        "categories": by_type,
        "quality": round(quality, 2),
        "avg_size": int(total_chars / max(len(rows), 1)),
        "similarity_pairs": max(0, len(rows) - len(by_type))
    }

def build_risk_summary(vendors, budget, resources, forecast):
    rejected = fetch_scalar("SELECT COUNT(*) FROM vendor_uploads WHERE status='Rejected'")
    pending = fetch_scalar("SELECT COUNT(*) FROM vendor_uploads WHERE status='Pending'")
    delayed = fetch_scalar("SELECT COUNT(*) FROM running_progress_metrics WHERE completion_percentage < 50 AND logged_date < DATE_SUB(CURDATE(), INTERVAL 30 DAY)")
    vendor_risk = clamp(sum(1 for vendor in vendors if vendor["overall"] < 55) * 18 + rejected * 5)
    budget_risk = clamp(pct(max(budget["variance"], 0), budget["allocated"]) * 8)
    resource_risk = clamp(resources["shortage_count"] * 20)
    delay_risk = clamp(float(forecast["delay_probability"] or 0) + delayed * 2)
    compliance_risk = clamp(pending * 6 + rejected * 12)
    quality_risk = clamp(100 - (sum(v["quality"] for v in vendors) / max(len(vendors), 1)))
    risks = [
        ("Project Delay Risk", delay_risk),
    ...
    return [{
        ...
        "recommendation": "Immediate executive review required" if score >= 70 else ("Monitor and assign mitigation owner" if score >= 40 else "Continue routine control")
    } for name, score in risks]

def build_executive_data():
    vendors = build_vendor_intelligence()
    resources = build_resource_summary()
    budget = build_budget_summary()
    forecast = build_completion_forecast()
    documents = build_document_intelligence()
    learning = build_learning_analytics()
    risks = build_risk_summary(vendors, budget, resources, forecast)
    total_vendors = fetch_scalar("SELECT COUNT(*) FROM users WHERE role IN ('vendor','admin')")
    active_vendors = sum(1 for vendor in vendors if vendor["drawings"] > 0 or vendor["dpr_performance"] > 0)
    total_drawings = fetch_scalar("SELECT COUNT(*) FROM vendor_uploads")
    approved_drawings = fetch_scalar("SELECT COUNT(*) FROM vendor_uploads WHERE status='Approved'")
    pending_drawings = fetch_scalar("SELECT COUNT(*) FROM vendor_uploads WHERE status='Pending'")
    dpr_count = fetch_scalar("SELECT COUNT(*) FROM progress_uploads")
    delayed_activities = fetch_scalar("SELECT COUNT(*) FROM running_progress_metrics WHERE completion_percentage < 50 AND logged_date < DATE_SUB(CURDATE(), INTERVAL 30 DAY)")
    critical_risks = sum(1 for risk in risks if risk["severity"] == "High")
    completion = forecast["overall_completion"]
    health_score = round(clamp((completion * .35) + (pct(approved_drawings, total_drawings) * .25) + ((100 - forecast["delay_probability"]) * .20) + (resources["avg_performance"] * .10) + (100 - pct(max(budget["variance"], 0), budget["allocated"])) * .10), 2)
    alerts = []
    if pending_drawings:
        alerts.append(f"{pending_drawings} drawing submissions are pending executive/admin review.")
    if delayed_activities:
        alerts.append(f"{delayed_activities} progress activities show delay exposure against DPR history.")
    if resources["shortage_count"]:
        alerts.append(f"{resources['shortage_count']} resource categories report shortage indicators.")
    if budget["variance"] > 0:
        alerts.append("Forecasted cost is above allocated budget in one or more departments.")
    if not alerts:
        alerts.append("No critical executive alerts currently detected.")
    return {
        "kpis": {
            ...
            "dpr_submissions": dpr_count,
            "overall_completion": completion,
            "health_score": health_score,
            "resource_status": resources["avg_performance"],
        ...
        "risks": risks,
        "alerts": alerts,
        "summary": f"Project completion is {completion}% with health score {health_score}%. {active_vendors} of {total_vendors} vendors are active, {dpr_count} DPR submissions are recorded, and the current delay probability is {forecast['delay_probability']}%."
    }

def answer_executive_query(question, data):
    q = (question or "").lower()
    if not q:
        return ""
    if "vendor" in q:
        top = data["vendors"][0] if data["vendors"] else None
        weak = [v for v in data["vendors"] if v["overall"] < 55]
        if top:
            return f"Vendor intelligence: top vendor is {top['vendor']} with overall score {top['overall']}%. Underperforming vendors: {', '.join(v['vendor'] for v in weak) if weak else 'none currently flagged'}."
    if "budget" in q or "cost" in q:
        return f"Budget intelligence: allocated budget is Rs {data['budget']['allocated']:,.0f}, utilized is Rs {data['budget']['utilized']:,.0f}, forecast variance is Rs {data['budget']['variance']:,.0f}, financial utilization is {data['budget']['utilization']}%."
    if "resource" in q:
        return f"Resource intelligence: average utilization is {data['resources']['avg_utilization']}%, performance score is {data['resources']['avg_performance']}%, and {data['resources']['shortage_count']} resource categories show shortages."
    if "risk" in q or "delay" in q:
        high = [risk for risk in data["risks"] if risk["severity"] == "High"]
        return f"Risk intelligence: delay probability is {data['forecast']['delay_probability']}%. High risks: {', '.join(r['name'] for r in high) if high else 'none'}."
    if "progress" in q or "completion" in q:
        return f"Progress forecast: overall completion is {data['forecast']['overall_completion']}%, estimated completion is {data['forecast']['estimated_completion']}, predicted remaining duration is {data['forecast']['remaining_duration']}."
    return data["summary"] + " Executive recommendation: focus daily review on delayed activities, pending drawings, resource shortages, and vendors below 55% overall score."
...
def executive_command_centre():
    if "username" not in session:
        return redirect("/login")
    return render_template("executive_command.html", data=build_executive_data(), active_section="command")

@app.route("/vendor_intelligence")
def vendor_intelligence():
    if "username" not in session:
        return redirect("/login")
    return render_template("executive_command.html", data=build_executive_data(), active_section="vendors")

@app.route("/resource_management")
def resource_management():
    if "username" not in session:
        return redirect("/login")
    return render_template("executive_command.html", data=build_executive_data(), active_section="resources")

@app.route("/budget_intelligence")
def budget_intelligence():
    if "username" not in session:
        return redirect("/login")
    return render_template("executive_command.html", data=build_executive_data(), active_section="budget")

@app.route("/prediction_engine")
def prediction_engine():
    if "username" not in session:
        return redirect("/login")
    return render_template("executive_command.html", data=build_executive_data(), active_section="prediction")

@app.route("/executive_ai", methods=["GET", "POST"])
def executive_ai():
    if "username" not in session:
        return redirect("/login")
    data = build_executive_data()
    question = request.form.get("question", "").strip() if request.method == "POST" else request.args.get("q", "").strip()
    answer = answer_executive_query(question, data)
    return render_template("executive_command.html", data=data, active_section="assistant", assistant_question=question, assistant_answer=answer)

@app.route("/learning_intelligence")
def learning_intelligence():
    if "username" not in session:
        return redirect("/login")
    return render_template("executive_command.html", data=build_executive_data(), active_section="learning")

@app.route("/drawing_intelligence")
def drawing_intelligence():
    if "username" not in session:
        return redirect("/login")
    return render_template("executive_command.html", data=build_executive_data(), active_section="documents")

@app.route("/risk_management")
def risk_management():
    if "username" not in session:
        return redirect("/login")
    return render_template("executive_command.html", data=build_executive_data(), active_section="risks")
...
def learning():
    ...
    learning_analytics = build_learning_analytics()
